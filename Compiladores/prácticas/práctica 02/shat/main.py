...
if __name__ == "__main__":
    ...
    productions = [Production('E', 'E + T'), Production('E', 'E - T'), Production('E', 'T'),
                   Production('T', 'T * F'), Production('T', 'T / F'), Production('T', 'F'),
                   Production('F', '( E )'), Production('F', 'n')]

    # Crear una instancia de la gramática
    grammar = Grammar(['+','*','-','/','n', '(', ')'], ['E','T','F'], 'E', productions)

    # Crear instancias de los parsers
    ll1_parser = LL1Parser(grammar)
    slr_parser = SLRParser(grammar)

    # Generar las tablas
    ll1_tables = ll1_parser.generate_table()  # Ahora espera ambas tablas
    ll1_action_table, ll1_goto_table = ll1_tables  # Desempaqueta ambas tablas
    ...
    for test_string in test_strings:
        ...
        # Realiza la lógica de análisis y muestra los resultados
